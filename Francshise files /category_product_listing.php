...
?></div>
      ...
      <tr>
        <td><table border="0" width="100%" cellspacing="0" cellpadding="0">
          <tr>
            <td valign="top" class="responsive-table"><table border="0" width="100%" cellspacing="0" cellpadding="2">
              <tr class="dataTableHeadingRow">
<?php if ($action == '') { ?>
                <td class="dataTableHeadingContent" width="20" align="right"><?php echo TABLE_HEADING_ID; ?></td>
                <td class="dataTableHeadingContent"><?php echo TABLE_HEADING_CATEGORIES_PRODUCTS; ?></td>
                <td class="dataTableHeadingContent">URI Mapping</td>
                ...
                <td class="dataTableHeadingContent" align="right"><?php echo TABLE_HEADING_CATEGORIES_MASTER_PRODUCT; ?></td>
<?php } // action == '' ?>
              </tr>
<?php
    ...
    if (isset($_GET['search'])) {
      ...
      $categories = $db->Execute("select c.categories_id, cd.categories_name, cd.categories_description, c.categories_image,
                                         ...
                                         c.categories_status
                                  from " . TABLE_CATEGORIES . " c, " . TABLE_CATEGORIES_DESCRIPTION . " cd, " . TABLE_PRODUCTS . " p
...
*/
      $products_query_raw = ("select p.products_type, p.products_id, pd.products_name, p.products_quantity,
                                ...
                                or p.display_attributes = '" . zen_db_input($_GET['search']) . "'
                                or p.products_model like '%" . zen_db_input($_GET['search']) . "%')" .
                                $order_by);
...
// Start modifications for Inline Quantity Update - May 7, 2015
...
// Split Page
?>
                <input type="hidden" class="previousStore" value="" />
                <td class="dataTableContent productID_block" width="20" align="right" <?php echo 'onclick="document.location.href=\'' . zen_href_link($type_handler , 'page=' . $_GET['page'] . '&product_type=' . $products->fields['products_type'] . '&cPath=' . $cPath . '&pID=' . $products->fields['products_id'] . '&action=new_product' . (isset($_GET['search']) ? '&search=' . $_GET['search'] : '')) . '\'"' ?> ><?php echo $products->fields['products_id']; ?></td>
                <td class="dataTableContent ctg_pdt_col"><?php echo '<a href="' . zen_href_link($type_handler, 'cPath=' . $cPath . '&pID=' . $products->fields['products_id'] . '&action=new_product_preview&read=only' . '&product_type=' . $products->fields['products_type'] . (isset($_GET['page']) ? '&page=' . $_GET['page'] : '')) . '">' . zen_image(DIR_WS_ICONS . 'preview.gif', ICON_PREVIEW) . '</a>&nbsp;<span>' . $products->fields['products_name']; ?></span></td>
                <?php 
                  if(isset($uri_lists))
                    $html = get_string_between($uri_lists[$products->fields['products_id']],'name="uri-mappings[1]" value="', '"'); 
                  else
                    $html = '';
                ?>
                <td class="dataTableContent uri_mapping_col"><?=$html?></td>                
                <td class="dataTableContent uri_mapping_event_col hidden">
                <div class="upo">
                  <?php echo zen_draw_form('update',FILENAME_CATEGORIES, 'action=update_product_option', 'post'); ?>  
                    <?=$uri_lists[$products->fields['products_id']]?>
                    <input type="hidden" name="products_id" value="<?=$products->fields['products_id']?>">
                    
                    <span class="btn-new updt-btn">Update<?php echo zen_image_submit('button_update.gif', IMAGE_UPDATE); ?></span>
                    <a href="javascript:void(0)"  class="close-modal btn-new"><img src="includes/languages/english/images/buttons/button_cancel.gif" border="0" alt="Cancel" title=" Cancel ">Cancel</a>
                  </form>
                  </div>
                </td>
                <td class="dataTableContent model_col"><?php echo $products->fields['products_model']; ?></td>
                <td colspan="2" class="dataTableContent price_col" align="right"><?php echo zen_get_products_display_price($products->fields['products_id']); ?></td>

		        <td class="dataTableContent" width="30" align="right">
				<?php  
				echo zen_draw_form('quantity', FILENAME_CATEGORIES, 'action=update_quantity&pID=' . $products->fields['products_id'] . '&cPath=' . $cPath, 'post');
				echo zen_draw_input_field('new_quantity', $products->fields['products_quantity'], 'style="width: 30px"'); 
				?>
				</form>
...
// End Modifications for Inline Quantity Update
      if ($products->fields['products_status'] == '1') {
        ...
        <input type="hidden" name="pID" value="<?php echo $products->fields['products_id']; ?>" />
        </form>
<?php
      } else {
        echo zen_draw_form('setflag_products', FILENAME_CATEGORIES, 'action=setflag&pID=' . $products->fields['products_id'] . '&cPath=' . $cPath . (isset($_GET['page']) ? '&page=' . $_GET['page'] : '') . ((isset($_GET['search']) && !empty($_GET['search'])) ? '&search=' . $_GET['search'] : ''));?>
...
<?php
  $sql_master_product = $db->Execute("SELECT * FROM " . TABLE_PRODUCTS . " WHERE display_attributes = '" . $products->fields['products_id'] . "'");
  if($sql_master_product->RecordCount() > 0) {
    $master_product_type = "Mater Product";
  } else {
    $sql_child_product = $db->Execute("SELECT * FROM " . TABLE_PRODUCTS . " WHERE products_id = '" . $products->fields['products_id'] . "'");
    if($sql_child_product->RecordCount() > 0) {
      $master_product_id = 0;
      while(!$sql_child_product->EOF) {
        $master_product_id = $sql_child_product->fields['display_attributes'];
        $sql_child_product->MoveNext();
      }
      if($master_product_id != 0) {
        $master_product_type = "Copied from Product# " . $master_product_id;
      } else {
        $master_product_type = '';
      }
    }
...
?>                
                <td class="dataTableContent" align="right"><?php echo $master_product_type; ?></td>
              </tr>
<?php
      $products->MoveNext();
    }
...
if ($action == '') { ?>
              <tr>
                <td colspan="12" style="padding-top:0; padding-bottom: 0;"><table border="0" width="100%" cellspacing="0" cellpadding="2">
                  <tr>
                    <td class="smallText" style="border: 0;"><?php echo TEXT_CATEGORIES . '&nbsp;' . $categories_count . '<br />' . TEXT_PRODUCTS . '&nbsp;' . $products_count; ?></td>
                    <td align="right" style="border-bottom: 0;border-right: 0;" class="smallText ad"><?php if (sizeof($cPath_array) > 0) echo '<a class="btn-new" href="' . zen_href_link(FILENAME_CATEGORIES, $cPath_back . 'cID=' . $current_category_id) . '">' . zen_image_button('button_back.gif', IMAGE_BACK) . 'Search</a>&nbsp;'; if (!isset($_GET['search'])) echo (!$zc_skip_categories ? '<a class="btn-new" href="' . zen_href_link(FILENAME_CATEGORIES, 'cPath=' . $cPath . '&action=new_category') . '">' . zen_image_button('button_new_category.gif', IMAGE_NEW_CATEGORY) . 'New Category</a>&nbsp;' : ''); ?>
