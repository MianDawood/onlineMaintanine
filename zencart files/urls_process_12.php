...
if (isset($_POST['ajax_action']) && $_POST['ajax_action'] === 'insert_qs_new_record') {
    ...
    // Helper function to safely get POST input
    function safe_input($key) {
        return isset($_POST[$key]) ? zen_db_input(trim($_POST[$key])) : '';
    }
    // Gather inputs
    $domain                = safe_input('domain');
    $qs_meta_title         = safe_input('qs_meta_title');
    $qs_meta_description   = safe_input('qs_meta_description');
    $cookiecutterqs_temp   = safe_input('cookiecutterqs_temp');
    ...
    if ($cookiecutterqs_temp === '' || !ctype_digit($cookiecutterqs_temp)) {
        echo json_encode([
            'status'  => 'error',
            'message' => 'Invalid or missing CookiecutterQS Template. Please select a valid template.'
        ]);
        exit;
    }
    // Validate required multi-value field
    if (!isset($_POST['qs_url']) || !is_array($_POST['qs_url']) || count($_POST['qs_url']) === 0) {
        echo json_encode([
            'status'  => 'error',
            'message' => 'Please select at least one QS URL.'
        ]);
        exit;
    }

    $qs_urls = $_POST['qs_url'];

    // Gather QS Textboxes 1-5
    $qs_textboxes = [];
    for ($i = 1; $i <= 5; $i++) {
        $qs_textboxes["qsTextbox$i"] = safe_input("qsTextbox$i");
    }
    // Prepare insert tracking
    $insertedCount = 0;
    $skippedCount  = 0;
    $errors        = [];

    foreach ($qs_urls as $qs_url_raw) {
        $qs_url = zen_db_input(trim($qs_url_raw));
        ...
        $check = $db->Execute(
            "SELECT COUNT(*) AS total FROM cookiecutterQS_qs_url 
             WHERE domain = '$domain' AND qs_url = '$qs_url'"
        );
        ...
        $sql = "
            INSERT INTO cookiecutterQS_qs_url (
                ...
                '$qs_meta_title',
                '$qs_meta_description',
                '{$qs_textboxes['qsTextbox1']}',
                '{$qs_textboxes['qsTextbox2']}',
                '{$qs_textboxes['qsTextbox3']}',
                '{$qs_textboxes['qsTextbox4']}',
                '{$qs_textboxes['qsTextbox5']}'
            )
        ";
        ...
        try {
            ...
            $errors[] = "Error inserting QS URL: $qs_url. Exception: " . $e->getMessage();
        }
    }
    ...
    if ($insertedCount > 0) {
        ...
        if ($skippedCount > 0) {
            $message .= " ⚠️ $skippedCount duplicate(s) skipped.";
        }
        echo json_encode([
            'status'  => 'success',
            'message' => $message,
        ...
        echo json_encode([
            'status'  => 'error',
            'message' => 'No new QS URL records inserted. Possible duplicates or missing data.',
            'errors'  => $errors
        ]);
    }
...
if (isset($_POST['ajax_action']) && $_POST['ajax_action'] == 'delete_template_field') {
  ...
  $qs_columns = array(
      'qsTextbox1', 'qsTextbox2', 'qsTextbox3', 'qsTextbox4', 'qsTextbox5','cookiecutter_id'
  );
...
if (isset($_POST['ajax_action']) && $_POST['ajax_action'] == 'save_template_field') {
  ...
  $qs_table_columns = array(
      'qsTextbox1', 'qsTextbox2', 'qsTextbox3', 'qsTextbox4', 'qsTextbox5','qs_meta_title','qs_meta_description','cookiecutter_id'
  );
  ...
  if (in_array($columnName, $template_table_columns)) {
     ...
     if( $db->Execute($sql)){
      echo json_encode(array('status' => 'success', 'displayText' => $newValue, 'message' => $columnName. ' (template_to_urls).'));
     }
      ...
      if($db->Execute($sql))
      {
        echo json_encode(array('status' => 'success', 'displayText' => $newValue, 'message' =>  $columnName. '(cookiecutterQS_qs_url).'));
      }
  } else {
      echo json_encode(array('status' => 'error', 'message' => 'Invalid column.'));
  }
...
if (isset($_GET['action']) && $_GET['action'] == 'get_urls_data') {
    ...
    if ($selected_table == 'template_to_urls') {
        ...
        // Construct SQL query based on domain
        if ($selected_domain == 'all') {
            $sql = "SELECT $fields FROM template_to_urls"; 
            $count_sql = "SELECT COUNT(*) as total FROM template_to_urls";
    
            // Add search condition if search_url is provided
            if (!empty($search_url)) {
                $sql .= " WHERE url LIKE '%" . zen_db_input($search_url) . "%'";
                $count_sql .= " WHERE url LIKE '%" . zen_db_input($search_url) . "%'";
            }
...
elseif ($selected_table == 'cookiecutterQS_qs_url') {
        ...
        if ($include_meta) {
            $fields .= ", cqs.qs_meta_title, cqs.qs_meta_description, c.name AS qs_cookiecutter_name,c.id AS cookiecutter_id";
        }
        ...
        // Adding search functionality for `qs_url`
            ...
            if (!empty($search_url)) {
                $sql .= " WHERE cqs.qs_url LIKE '%" . zen_db_input($search_url) . "%'";
                $count_sql .= " WHERE cqs.qs_url LIKE '%" . zen_db_input($search_url) . "%'";
            }
