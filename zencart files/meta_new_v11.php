...
if (isset($_POST['ajax_action']) && $_POST['ajax_action'] == 'delete_template_field') {
  ...
  $allowed_columns = array('template', 'template1', 'template2', 'template3', 'template4', 'template5', 'cookiecutterqs_temp','textbox1', 'textbox2', 'textbox3', 'textbox4', 'textbox5');
...
if (isset($_POST['ajax_action']) && $_POST['ajax_action'] == 'save_template_field') {
  ...
  $allowed_columns = array('template', 'template1', 'template2', 'template3', 'template4', 'template5','textbox1', 'textbox2', 'textbox3', 'textbox4', 'textbox5',
        ...
        <?php foreach ($row as $column_name => $cell): ?>
            <td id="template_cell_<?php echo htmlspecialchars($row['id']) . '_' . htmlspecialchars($column_name); ?>">
                ...
                $template_fields = array(
                    'template', 'template1', 'template2', 'template3', 'template4', 'template5','textbox1', 'textbox2', 'textbox3', 'textbox4', 'textbox5'
                );
                ...
                if ($column_name === 'cookiecutterqs_temp' && isset($form_data['include_templates'])) {
                  ...
                  (in_array($column_name, ['textbox1', 'textbox2', 'textbox3', 'textbox4', 'textbox5']) && isset($form_data['include_textboxes']))
              ) {
                    // Show value
                    echo nl2br(htmlspecialchars($cell));
  ...
  <ul class="pagination">
    <?php for ($i = 1; $i <= $total_pages; $i++): ?>
      <li class="page-item <?= ($i == $page) ? 'active' : '' ?>">
        <a class="page-link" href="meta_new.php?action=get_urls_data&page=<?= $i ?>"><?= $i ?></a>
      </li>
    <?php endfor; ?>
  </ul>
</nav>
...
function viewSelectedTemplate() {
  ...
  if (res.result === 'success') {
    ...
    // Froala for HTML view
    if (window.viewEditor) {
      window.viewEditor.destroy(); // Avoid duplicate editors
    }
    $('#template_html_editor').html('');
    window.viewEditor = new FroalaEditor('#template_html_editor', {
...
function edit_each_temp(cell, domain, rowid) {
  ...
  $.post('meta_new.php', req_data, function (res) {
    ...
    // Fallback content if template is empty or missing
    if (!convertedHtml || !convertedHtml.trim()) {
      convertedHtml = '<p style="color: gray; text-align: center;">No template available to preview</p>';
    }

    // Destroy previous editor instance and clear container
    if (window.editor) {
      window.editor.destroy();
      $('#froala_editor').html('');
    }

    // Show modal first — always
    $('#update_template_css_modal').modal('show');

    // Once modal is visible, initialize the editor with the fallback or actual content
    $('#update_template_css_modal').off('shown.bs.modal').on('shown.bs.modal', function () {
      $('#template_edit_section').show(); // Ensure the section is visible
