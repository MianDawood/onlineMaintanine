...
<?php } else{?>
  ...
  <?php 
    foreach($files as $file) {
      if ($file['text'] == '--choose the template--') continue;

      $val = htmlspecialchars($file['text']);
      $id = htmlspecialchars($file['id']);
      $screenshot_name = str_replace('.php','.png', $file['text']);
      $imagePath = file_exists('../../cdn/design_block/photos/'.$screenshot_name)
                   ? '../../cdn/design_block/photos/'.$screenshot_name
                   : '../../cdn/design_block/photos/default.jpg';

      $status = $file['status'];
      $imageAttr = ($status == 1) ? "data-image='$imagePath'" : "";
      
      echo "<option value='$val' $imageAttr>$val</option>";
    }
  ?>
</select>
...
$(document).ready(function() {
  $("#choose_template").msDropdown({
    byJson: false
  });

  // On change, call your showValue function
  $("#choose_template").on("change", function() {
    const selectedText = $("#choose_template option:selected").text();
