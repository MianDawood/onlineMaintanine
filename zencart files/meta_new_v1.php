  ...
  <div class="modal-dialog modal-sm" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="templateEditModalLabel">Select Template</h5>
      ...
      <div class="modal-body">
        ...
        while (!$templates->EOF) {
            $za_template_names[] = array(
                'id' => $templates->fields['template_name'], // Use name as value
                'text' => $templates->fields['template_name']
            );
            $templates->MoveNext();
        }
...
<div class="modal fade" id="cookieqsModal" tabindex="-1" role="dialog" aria-labelledby="cookieqsModalLabel" aria-hidden="true">
    ...
    <form id="cookieqs_form" onsubmit="return saveCookieQSValue();">
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <select id="cookieqs_dropdown" name="newValue" class="form-control select2">
            <?php
...
function saveTemplateSelection() {
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            // Rebuild the cell HTML
            var newHtml = nl2br(selectedTemplate);
            ...
            if (selectedTemplate.trim() != '') {
                ...
                newHtml += ' <button class="btn btn-xs btn-outline-primary" style="padding:2px 5px; font-size:10px;" title="Edit" ' +
                    'onclick="openTemplateModal(\'edit\', \'' + currentTemplateField + '\', \'' + currentDomain + '\', \'' + selectedTemplate + '\', \'' + currentRowId + '\')">✏️</button>';
...
function saveCookieQSValue() {
  ...
  $.post('meta_new.php', data, function(response) {
    ...
    if (res.status === 'success') {
      ...
      alert('Template ' + actionType + ' saved and updated!');
      const cellId = `#template_cell_${data.rowId}_${data.columnName}`;
      $(cellId).html(`${$('#cookieqs_dropdown option:selected').text()} 
        <button class="btn btn-xs btn-outline-primary" title="Edit" onclick="openCookieQSModal('edit', '${data.columnName}', '${data.domain}', '${data.newValue}', '${data.rowId}')">✏️</button>
